...
function calculateAge(dateentered) {
    ...
    const timeNowDay = timeNow.getDate();
...
button === null || button === void 0 ? void 0 : button.addEventListener("click", () => {
    let inputvalue = document.getElementById("input-here");
    let anspara = document.getElementById("get-age");
    let dateentered = inputvalue === null || inputvalue === void 0 ? void 0 : inputvalue.value;
    //first type cast and then use it.
    if (dateentered) {
        const parsedDate = new Date(dateentered);
        if (!isNaN(parsedDate.getTime())) {
            const age = calculateAge(parsedDate);
            anspara.innerText = `Age: ${age.years} years, ${age.months} months, and ${age.days} days`;
        }
        else {
            anspara.innerText = "Invalid date format";
        }
    }
    else {
        console.log(`please select the date`);
        anspara.innerText = "Please Select the date";
    }
});
